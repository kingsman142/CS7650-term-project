...
device = torch.device("cuda:0" if hyper_params["cuda"] else "cpu")
...
#train_dataset, _ = torch.utils.data.random_split(train_dataset, [100, 87205])
valid_dataset = SquadDataset(d_w_context, d_c_context, d_w_question, d_c_question, d_labels)
#valid_dataset, _ = torch.utils.data.random_split(valid_dataset, [100, 10366])
...
valid_dataloader = DataLoader(valid_dataset,
                              shuffle=True,
                              batch_size=hyper_params["batch_size"], num_workers = 0) # num_workers = 4
...
model = BiDAF(word_vectors=word_embedding_matrix,
              ...
              drop_prob=hyper_params["drop_prob"]).to(device)
...
for epoch in range(hyper_params["num_epochs"]):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        try:
            ...
            if i % 100 == 0:
                print("(train) Epoch {}/{} -- Batch {}/{} complete".format(epoch+1, hyper_params["num_epochs"], i+1, len(train_dataloader)))
        except:
            training_failures += 1
            pass
    ...
    with torch.no_grad():
        for i, batch in enumerate(valid_dataloader):
            ...
            try:
                ...
                if i % 100 == 0:
                    print("(validation) Epoch {}/{} -- Batch {}/{} complete".format(epoch+1, hyper_params["num_epochs"], i+1, len(valid_dataloader)))
            except:
                val_failures += 1
                pass
...
print("# training failures: {}".format(training_failures))
print("# validation failures: {}".format(val_failures))
